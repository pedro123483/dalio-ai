...
// Função para adicionar um pequeno delay (útil para mostrar o loading de PDF)
const delay = (ms: number) => new Promise(resolve => setTimeout(resolve, ms));

export async function POST(req: Request) {
  const { messages } = await req.json();

  // Verificar se há anexos (arquivos) na última mensagem
  const lastMessage = messages[messages.length - 1];
  let hasPdf = false;
  let systemPrompt = "";
  
  // Verificar se há anexos de PDF
  ...
  // Sistema base
  systemPrompt = `Você é um assistente financeiro especializado em análise de ativos do mercado brasileiro. Quando solicitar informações sobre algum ativo, SEMPRE forneça uma análise detalhada dos dados apresentados, explicando o significado dos valores e possíveis implicações.
  ...
  // Se for PDF, adicionar instrução específica
  if (hasPdf) {
    systemPrompt += `\n\nO usuário anexou um arquivo PDF com informações financeiras. Analise o documento e forneça insights detalhados sobre os dados apresentados.`;
    
    // Adicionar um pequeno atraso para mostrar o ícone de loading
    await delay(2500);
  }
